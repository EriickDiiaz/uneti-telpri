...
<div class="d-flex">
    <h2><i class="fa-regular fa-address-card m-2"></i>Modificar Rol.</h2>
</div>
...
<form action="{{ route('roles.update', $role) }}" method="POST">
    ...
    <div>
        ...
        <div class="col-sm-5">
            <input type="text" class="form-control shadow-sm" name="name" id="name" value="{{ old('name', $role->name) }}" required>
        </div>
    </div>
                    ...
                    <div class="col">
                        <div class="form-check border rounded p-2 shadow-sm">
                            <input class="form-check-input mx-2" type="checkbox" name="permissions[]" value="{{ $permission->id }}" id="permission{{ $permission->id }}"
                                {{ in_array((string) $permission->id, (array) old('permissions', $role->permissions->pluck('id')->map(fn($id) => (string) $id)->toArray()), true) ? 'checked' : '' }}>
